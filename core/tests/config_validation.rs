...
fn test_us_region_config_parsing() {
    ...
    let data = fs::read_to_string("../../Psychohistory/scenarios/simulation_config.json")
        .expect("Failed to read ../../Psychohistory/scenarios/simulation_config.json (run from core/tests)");
    ...
    let sim_params = us.get("simulation_parameters")
        .or_else(|| us.get("components").and_then(|c| c.get("simulation_parameters")))
    ...
    assert!(config.is_ok(), "SimulationConfig failed to parse for 'us': {config:?}");
}
...
fn test_all_regions_config_parsing() {
    ...
    for (name, region) in regions {
        let sim_params = region.get("simulation_parameters")
            .or_else(|| region.get("components").and_then(|c| c.get("simulation_parameters")));
        if let Some(params) = sim_params {
            let config: Result<SimulationConfig, _> = serde_json::from_value(params.clone());
            assert!(config.is_ok(), "SimulationConfig failed to parse for region '{name}': {config:?}");
        }
    }
}
