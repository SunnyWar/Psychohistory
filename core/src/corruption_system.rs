...
pub fn corruption_system(
    ...
    acc = (1.0 - avg_integrity).mul_add(
        registry.get_influence("gov:avg_integrity", "gov:corruption"),
        acc,
    );
    acc = lobbying_pressure.mul_add(
        registry.get_influence("gov:lobbying_pressure", "gov:corruption"),
        acc,
    );
    acc = donor_pressure.mul_add(
        registry.get_influence("gov:donor_pressure", "gov:corruption"),
        acc,
    );
    acc = reelection_pressure.mul_add(
        registry.get_influence("gov:reelection_pressure", "gov:corruption"),
        acc,
    );
    acc = normalized_wealth_influence.mul_add(
        registry.get_influence("gov:normalized_wealth_influence", "gov:corruption"),
        acc,
    );
    acc = faction_formation.mul_add(
        registry.get_influence("gov:faction_formation", "gov:corruption"),
        acc,
    );
    acc = bad_law_drag.mul_add(
        registry.get_influence("law:bad_law_drag", "gov:corruption"),
        acc,
    );
